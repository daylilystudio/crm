      ...
      <el-form label-position="left">
        <div class="grid sm:gap-x-8 sm:grid-cols-2">
          ...
          <el-form-item label="部門類別名" prop="name">
            <el-input v-model="filterForm.name" placeholder="輸入部門類別名" clearable />
          </el-form-item>
        </div>
    ...
    <el-card class="box-card" v-loading="store.state.progressing">
      <template #header>
        <div class="card-header flex justify-between items-center">
          <span>查詢結果</span>
          <el-button v-if="store.state.show.create" @click="store.dispatch('action', {type:'add', id:0})" :icon="Plus" type="primary" plain>新增部門類別</el-button>
        </div>
      </template>
