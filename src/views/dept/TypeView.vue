<template>
      ...
      <el-form label-position="left">
        <div class="grid sm:gap-x-8 sm:grid-cols-2">
          ...
          <el-form-item label="部門類別名" prop="name">
            <el-input v-model="filterForm.name" placeholder="Enter 部門類別名" clearable />
          </el-form-item>
        </div>
    ...
    <el-card class="box-card" v-loading="store.state.progressing">
      <template #header>
        <div class="card-header flex justify-between items-center">
          <span>Result</span>
          <el-button v-if="store.state.show.create" @click="store.dispatch('action', {type:'add', id:0})" :icon="Plus" type="primary" plain>Add部門類別</el-button>
        </div>
      </template>
      <el-table :data="store.state.table" :flexible="true" border>
        <el-table-column prop="name" label="部門類別名" />
        <el-table-column prop="grading" label="等級" />
  ...
  <el-dialog v-model="store.state.modalOpen" :title="store.getters.actionTitle" :append-to-body="true" :close-on-click-modal="false" width="95%" style="max-width: 600px;">
    <TypeForm :key="store.state.actionType+store.state.actionId" @toggle-modal="v=>store.commit('updateModalOpen', v)" :type="store.state.actionType" :id="store.state.actionId" :path="path" />
  </el-dialog>
</template>
...
import TypeForm from '../../components/dept/TypeForm.vue'
