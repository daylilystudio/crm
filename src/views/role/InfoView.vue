    ...
    <el-card class="box-card" v-loading="store.state.progressing">
      <template #header>
        <div class="card-header flex justify-between items-center">
          <span>查詢結果</span>
          <el-button v-if="store.state.show.create&&store.state.user.superUser" @click="store.dispatch('action', {type:'add', id:0})" :icon="Plus" type="primary" plain>新增角色</el-button>
        </div>
      </template>
